  ...
  try {
    ...
    filesWithLabValues.forEach((file) => {
      ...
      if (file.labValues instanceof Map) {
        labEntries = Array.from(file.labValues.entries());
      } else {
      ...
      labEntries.forEach(([biomarkerName, processedBiomarkerData]) => {
        ...
        biomarkerHistory[biomarkerName].push({
          ...
          category: processedBiomarkerData.category || 'unknown'
        });
      });
    });
    ...
    res.render("user/reports", {
      ...
      initialData: `<script>window.__INITIAL_DATA__ = ${JSON.stringify({
        files: user.files,
        biomarkers: Object.keys(enrichedBiomarkerData), // Use enriched data
        biomarkerInfo: enrichedBiomarkerData, // Use enriched data
      })};</script>`,
    });
  } catch (error) {
...
// In routes/api.js (or wherever your RAG routes are)

router.post('/api/rag/conversations', async (req, res) => {
  try {
    ...
    // Import the conversation handlers
    const conversationHandlers = require('../controllers/conversations');
    
    switch (operation) {
      case 'list':
        return await conversationHandlers.handleList(req, res);
      case 'save':
        return await conversationHandlers.handleSave(req, res);
      case 'delete':
        return await conversationHandlers.handleDelete(req, res);
      default:
        return res.status(400).json({
